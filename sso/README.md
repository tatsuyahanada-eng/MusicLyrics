...
> **導入作業の手順は [SETUP.md](SETUP.md) にまとめてあります。**
> サーバーの準備・データベースの作成・ファイルの配置・アプリの切り替え・切り戻しまで、
> コマンド付きで順番に書いています。
...
**各アプリのURLはこれまでどおりで構いません。**
「必ずポータルを開いてから各アプリへ」という運用にする必要はなく、
「各アプリのトップページを認証サーバーに差し替える」必要もありません。

未ログインのときだけ自動的に認証サーバーを経由します。

| 利用者の状態 | 何が起きるか |
|---|---|
| 未ログイン | 開いた瞬間に認証サーバーへ転送 → ログイン → **元の開こうとしたページへ戻る** |
| ログイン済み | 認証サーバーを一瞬経由するが画面は出ず、そのままページが開く |
| 権限が無い | 「閲覧が許可されていません」と表示し、アプリの中身は表示しない |

保護したページを開くたびにガードが働くので、**URLを直接叩かれても素通りはできません**。
そのうえで、利用者にポータル画面を毎回見せる必要はない、というのがこの方式です。

ポータル（`https://auth.example.com/`）は「自分が使えるアプリの一覧」として併設できます。
実際の現場では、各アプリのヘッダーにポータルへのリンクを1本置く形が使いやすいはずです。
詳しくは [SETUP.md の「1. 先に決めること」](SETUP.md#1-先に決めることどちらの運用にするか) を参照してください。
...
> ここでは要点だけを示します。所有者・パーミッション・ウェブサーバー設定・
> 切り戻しまで含めた実作業の手順は [SETUP.md](SETUP.md) を参照してください。
...
└── sso/
    ...
    ├── sso_callback.php   ← client/sso_callback.php
...
ベースは白。ポイントとなる色は Los Angeles Lakers の配色をモチーフに、
**濃い紫・ゴールド・黒・白**の4色で統一しています。
色を足したいときは、この表の役割に合わせて使ってください。

| 色 | 値 | 使いどころ |
|---|---|---|
| Lakers Purple | `#552583` | 見出し、主要ボタン、選択中のメニュー、リンク |
| Deep Purple | `#3B1560` / `#2B0F4A` | ボタンのグラデーション、フッターの地色 |
| Lakers Gold | `#FDB927` | ヘッダー下の罫線、見出し脇の縦線、追加ボタン、「許可」「有効」バッジ |
| Black | `#17161A` | 「拒否」「停止中」バッジ、削除ボタン、警告帯 |
| White | `#FFFFFF` | 画面のベース、カードの地色 |

CSS 変数は `public/assets/style.css` の先頭にまとめてあるので、
色を変えたい場合はそこだけ書き換えれば全画面に反映されます。

> ゴールドは白地に対するコントラストが低いため、**文字色には使わず**、
> 必ず背景や罫線として使ってください（バッジは濃い紫の文字を載せています）。
...
```
sso/
├── README.md               この文書（仕組みと機能の説明）
├── SETUP.md                導入手順書（サーバー構築からアプリ切り替えまで）
