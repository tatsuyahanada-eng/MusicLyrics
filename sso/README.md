...
| `is_admin` | 管理者（このUser Management自体の管理者）かどうか |
| `role` | **そのアプリでの役割**（自由記述。管理画面で設定。後述） |
| `external_user_id` | **そのアプリが元々持っているユーザーID**（対応付けた場合） |

---

## アプリごとの役割（viewer / editor / admin など）

「見るだけの人」「編集できる人」「そのアプリの管理者」のように、**アプリごとに権限の段階を分けたい**場合は `role` を使います。

- 値は完全に自由入力です（`viewer` / `editor` / `admin` でも、`閲覧` / `編集` のような日本語でも構いません）。**SSOは文字列の中身を一切解釈しません**
- **「どのユーザーに、どの役割を割り当てるか」は、この管理画面（ユーザーの編集 → アプリの閲覧許可）で一元管理**します
- **その役割で実際に何ができる／できないかを決めて制御するのは、各PWA側のコードです。** SSOは役割名を運ぶだけで、業務ロジックには関与しません

設定は、ユーザーの編集画面の「アプリの閲覧許可」表にある「役割」欄に入力するだけです。閲覧が「許可」または「拒否」に**明示設定されている行にだけ**役割を付けられます（「既定」のままだと保存されません）。

アプリ側では、いつもの `$SSO_USER` に `role` が増えるだけです。

```php
<?php require __DIR__ . '/sso/sso_guard.php'; ?>

<?php if ($SSO_USER['role'] === 'admin'): ?>
  <a href="/admin/">管理画面</a>
<?php endif; ?>

<?php if (in_array($SSO_USER['role'], ['editor', 'admin'], true)): ?>
  <button>編集する</button>
<?php else: ?>
  <p>閲覧のみ許可されています</p>
<?php endif; ?>
...
権限マトリクスやCSV一括登録は、今まで通り「許可／拒否」だけを扱います（`role` はユーザー編集画面からのみ設定します）。既存の役割を消さずに、許可／拒否だけをまとめて変更できます。

### 既存アプリが元々持っている権限との関係

`role`（今回の新機能・SSO側で一元管理）と `external_user_id`（既存アプリのIDとの対応付け）は、別々の仕組みです。

- `external_user_id` は、**アプリが元々持っていた権限データへの「橋渡し」**です。SSOはこの値をそのまま渡すだけで、実際に古い権限を引っ張ってきて使うかどうかは、完全にアプリ側のコード次第です（「IDが同じだから自動で権限が引き継がれる」ということは起きません）
- 移行が終わったら、`external_user_id` を使うのをやめて、新しい `role` だけに一本化することもできます
...
- **新しく登録したアプリを、今いる全員に一度に見せたい場合**：
  **アプリ** → 対象のアプリを開く → 「ユーザーへの一括許可」→
  「全員に一括で許可を出す」。現在有効な全ユーザーに一度で許可が付きます
  （逆に「全員の個別設定を解除」で、既定ポリシーに戻すこともできます）
...
| 管理コンソールの入室制限 | `is_admin` フラグに加えて、`config.php` の `admin_usernames` で許可した<br>ログインIDだけが実際に入れる（後述）。未設定なら従来どおり `is_admin` のみで判定 |

---

## 管理コンソールに入れる人を絞り込む

既定では、ユーザー編集画面で「管理者にする」にチェックを入れた人は誰でも
この管理コンソール（ユーザー・アプリ・権限マトリクス・監査ログ）に入れます。

**特定のログインIDだけに絞りたい場合**は、`config.php` に次のように書きます。

```php
'admin_usernames' => ['welsysadmin'],
```

これを設定すると、**`is_admin` にチェックが入っていても、ここに載っていない
ログインIDは管理コンソールに入れなくなります。** 管理画面の操作ミスや、
万が一の設定変更で他のユーザーに `is_admin` が付いてしまっても、
この一覧に無ければブロックされる、という二重のロックになります。

複数人に管理を任せたい場合は、配列にログインIDを並べてください。

```php
'admin_usernames' => ['welsysadmin', 'it-support'],
```

`admin_usernames` を書かない（既定）場合は、これまでどおり `is_admin` のみで判定します。
