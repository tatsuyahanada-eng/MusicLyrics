...
3. **📋 コマンド一覧** — 「Monstera/Relier シミュレーター」（機器の操作パネルのシミュレーター）。
   1枚のカードにまとまっていて、シミュレーターのすぐ下にある調べたい保守機能のプルダウンで選ぶと、
   その操作が役立つ場面（登録されていれば）と、押すボタンが「9（2回） → 8 → 実行 → 完了」のように
   順番に表示され、右側には同じ手順を文章で読める「簡易手順」も出ます。「順番に再生」でハイライトしながら
   確認できます。画面の中のボタンは実際に押せて、
   **長押し・押しまま（離さずに次へ）・2つ同時押し**（◁と▷を一緒に長押し、など）・
   **同じボタンを連続で何回か押す**操作にも対応し、それぞれ色分けしてハイライトします。
   画面のそば（広い画面では左、狭い画面では上）には実機側面の**電源スイッチ**（上＝ON／下＝OFF）もあり、
...
4. **⚙️ 設定** — 業態・時間帯ごとの定型文（並び替え可）、よく使う文、**入店連絡に足す文**、
