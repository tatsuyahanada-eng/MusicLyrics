...
3. **📋 コマンド一覧** — 機器（SA-2610 マルチステーション）の操作パネルのシミュレーター。
   調べたい保守機能をプルダウンで選ぶと、その操作が役立つ場面（登録されていれば）と、
   押すボタンが「9（2回） → 8 → 実行 → 完了」のように順番に表示され、右側には同じ手順を文章で読める
   「簡易手順」も出ます。「順番に再生」でハイライトしながら確認できます。画面の中のボタンは実際に押せて、
   **長押し・押しまま（離さずに次へ）・2つ同時押し**（◁と▷を一緒に長押し、など）・
   **同じボタンを連続で何回か押す**操作にも対応し、それぞれ色分けしてハイライトします。
   画面の左には実機側面の**電源スイッチ**（上＝ON／下＝OFF）もあり、キーパッドの他のボタンと
   組み合わせた同時押し・押しままの手順にそのまま使えます。
