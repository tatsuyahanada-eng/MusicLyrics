...
3. **📋 コマンド一覧** — 機器（SA-2610 マルチステーション）の操作パネルのシミュレーター。
   ...
   **長押し・2つ同時押し**（◁と▷を一緒に長押し、など）・**同じボタンを連続で何回か押す**操作にも対応し、
   それぞれ色分けしてハイライトします。画面が変わらず結果だけ起こる操作（伝票の印刷など）は
   コメントで案内されます。待機中の画面は複数登録してプルダウンで切り替えられ（二重化・メイン単独・
   サブ単独・電源オフなど）、手順は「複製」ボタンで似た操作をコピーしてから編集でき、
