...
5. **⚙️ 設定** — 業態・時間帯ごとの定型文（並び替え可）、よく使う文、**入店連絡に足す文**、
   コマンド一覧（保守機能の操作手順）などを編集。**コマンドの手順は、実機シミュレーターと同じ
   キーパッドをタップして組み立てられます**（同時押し・長押し・押しままにも対応、同じボタンを
   続けてタップすると自動で「連続◯回」にまとまります）。
