 ...
 *
 * このアプリは複数人で使うことを前提にしています。担当者一人ひとりが、
 * 自分自身のGoogleアカウントで下記の手順を行ってください（コードは全員共通）。
 *
 * 導入手順
 *  1. 自分自身の https://script.google.com/ で新しいプロジェクトを作る
 *  2. このコードを貼り付けて保存
 *  3. 「デプロイ」→「新しいデプロイ」→ 種類は「ウェブアプリ」
 *  4. 次のユーザーとして実行: 自分
 *     アクセスできるユーザー: 全員
 *     （これで自分自身のカレンダーを返すURLになります）
 *  5. 表示された /exec で終わるURLを、このツールの「⚙️ 設定」→「担当者」欄の
 *     自分の行にある「この担当者ご本人のGoogleカレンダー連携URL」に貼り付ける
 */
function doGet(e) {
