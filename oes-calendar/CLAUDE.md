...
- **「📋 コマンド一覧」タブは、実機（SEIKO SA-2610 マルチステーション）の操作パネルのシミュレーター。**
  ...
  - **`SIM_KEYS`のidは手順データに保存される値なので、あとから変えない**（変えると登録済みの手順が壊れる）。
    ...
    **「シフト」「メニュー」は文字数が多く既定の大きさ（13.5px）だと折り返してしまうため、
    ...
    LCD（`.sim-lcd-box`）は白い実際の表示部だけで、`SEIKO`のブランド名やモデル名（`SA-2610`）は
  ...
  - **操作の入り口は2つあり、どちらも同じ手順データを使う。**
    ...
    **シミュレーターの動き（実質的な「動画」）だけでなく、手順を上から順番に読める文章のリストも
    右側に出す**（`#cmd-textlist` / `renderCmdTextList()`。②のカードを`.cmd-guide-body`で左右2列にし、
    左＝実機シミュレーター用の操作、右＝簡易手順のテキスト。画面が狭いときは縦に並ぶ）。
...
3. **📋 コマンド一覧** — 実機（SEIKO SA-2610 マルチステーション）の操作パネルのシミュレーター。
   ①「機器の画面（シミュレーター）」カード（`.sim-panel`）と②「操作を選んで手順を見る」カード
   （`#cmd-select`）の2枚。詳細は「重要なルール」の該当項目を参照。
...
| `openStoreChat()` | 一般のGoogle Chatを名前つきウィンドウで開き、「業態 店舗名」をクリップボードへコピーする（`#btn-open-chat`と「よく使う文」欄前のボタンの2箇所から呼ぶ。呼び出し元ごとに処理を分けない） |
...
（「今日の作業」カードと「よく使う文」欄の直前。どちらを押しても押すたびにChatを開き直しつつ
「業態 店舗名」をクリップボードへコピーすること）、③退店連絡のコピーボタンが特記事項の下にあること、
...
**②のカード右側に「簡易手順」の文章リストが出て、いま案内中の行がチップと同じタイミングで
