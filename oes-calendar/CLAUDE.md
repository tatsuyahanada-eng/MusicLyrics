...
- **作業当日タブの①②③の各「コピー」ボタンの隣に「Chat」ボタンを置く。** 押すと、その作業の**業態＋店舗名**
  ...
  **同じ店舗のあいだは `.closed` を見て開き直し可否を判定しないこと。** Google Chatは別オリジンのため、
  ...
  （利用者から実際に報告された不具合①）。
  **さらに、保存しておいたJS参照（`chatWin`）に対して `.focus()` を呼ぶ方式にも戻さないこと。**
  一度でも別オリジン（Google Chat）へ移動した`chatWin`は、COOPの影響で `.focus()` が例外も投げずに
  何も起きない「無反応」になることがあり、実際に**Chatボタンが完全に反応しなくなる不具合**が起きた
  （利用者から実際に報告された不具合②）。そのため `openStoreChat()` は、保存済みのJS参照を信用せず、
  **毎回 `window.open('', 'oes-store-chat')` で名前つきウィンドウを取りに行く**方式にする。
  この呼び方はブラウザが「同名のウィンドウがあればURLを変えずにそれを返す（＝再読み込みされない）、
  無ければ空白の新規ウィンドウを開く」という、JS参照の生死に左右されない仕組みなので、
  COOPで参照が壊れていても確実に反応する。返ってきたウィンドウが**まだ何も読み込んでいない
  （`location.href` が `about:blank`）か、業態＋店舗が前回と変わっていれば**、そのときだけ
  `location.href = url` で移動させる。読み取りが例外になる（＝すでに別オリジンへ移動済み＝開いたまま）
  場合は移動させず `focus()` のみ行う。
- **Chatは毎回新しいタブを開かず、名前付きウィンドウ（`oes-store-chat`）1枚を使い回す。**
  さらに**直前と同じ業態＋店舗であればURLを読み込み直さず、そのタブを前面に出すだけ**にする
  （`chatWinKey`で業態＋店舗の変化だけを見る。`chatWin`自体は参照が壊れる前提でロジックに使わない）。
  一覧から探して開いた状態がそのまま残り、毎回探し直さずに済むため。
