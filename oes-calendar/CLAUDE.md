...
- **「📄 レポートの見方」タブは、実機が印字する「MONSTERA インストールレポート」の読み方を
  ...
  - 章（インストール情報／システム情報／ECT／MST／MPR／MD／HT）は`.rg-chip`のナビゲーションで
    切り替える（`repSecId`はレンダリングとは無関係なローカル変数、共有設定には入れない）。
...
| `openRepTab()` / `renderRepNav()` / `selectRepSection(id)` / `repSecId` | レポートの見方タブの初期化・章ナビ・章の切り替え（`repSecId`は共有設定に入れない） |
| `renderRepSection()` / `repLineHtml(line,li)` | 印字例・項目一覧・注意書きの描画。`tok.p`の桁位置どおりに空白を入れて実機と同じ桁に並べる |
...
| `repHlKey(k)` / `repClearHl()` / `repFieldName(sec,k)` | 記号（A・B・C…）で印字例の値と「項目一覧」を相互にハイライト連動させる |
...
レポートの見方タブは、章のナビゲーション（7章）で切り替わること、**行にマウスを乗せるとその行の説明**が、
**値にマウスを乗せるとその項目だけの詳しい説明**が出ること（`mouseover`だけでなく**タップ（click）でも
同じように動く**こと）、記号（A・B・C…）付きの章で値と「項目一覧」がハイライトで連動すること、
