...
- **「📋 コマンド一覧」タブは、実機（SEIKO SA-2610 マルチステーション）の操作パネルのシミュレーター。**
  ...
  - **`SIM_KEYS`のidは手順データに保存される値なので、あとから変えない**（変えると登録済みの手順が壊れる）。
    ...
    **キーの色は実機の見た目を再現するための例外**（アプリ全体の2色ルールとは別）：
    F・△・シフト・◁・電源・クリアは画面と同じ黒（`.k-black`）、メニュー・実行だけは茶色（`.k-gold`）、
    それ以外（数字・▷・▽）は既定の明るい色のまま。
  ...
  - **ハイライトは押し方で色分けする**（`.simkey.on`＝赤、`.simkey.on.hold-mode`＝長押しは琥珀色、
    `.simkey.on.rep-mode`＝連続押しは青、連続押し中はキーの上に`.rep-badge`で「今何回目/全部で何回」を表示）。
    同じ色分けを手順チップ（`.cmd-step`）にも適用している。
  - **待機中（何も操作していない）の画面文字は`settings.simIdleL1`/`simIdleL2`で管理者が自由に書き換えられる**
    （実機は機種や設定によって「[01]」や時刻が出ないこともあるため、ハードコードをやめて設定に出した）。
    文字列中の`{time}`だけが現在時刻に置き換わる（`applyIdleTokens()`）。時刻を出したくなければ`{time}`を
    書かなければよい。`simDefaultScreen()`はCLEARを押したときや、操作の選択・リセット時にも呼ばれる。
  ...
  - **CSVで一括登録・書き出しができる**（`exportCommandsCsv()` / `importCommandsCsv()` / 手書きの`parseCsv()`）。
    ...
    **読み込むと`settings.commands`は丸ごと置き換わる**（`confirm()`で確認してから）。
...
| `simSetScreen(l1,l2)` / `simDefaultScreen()` / `simSetMsg(msg)` / `simSetNote(memo)` / `applyIdleTokens(s)` | シミュレーターのLCD表示（上下行）・待機画面（`{time}`置換）・案内メッセージ・結果コメント欄 |
...
| `exportCommandsCsv()` / `importCommandsCsv(text)` / `parseCsv(text)` | コマンド一覧のCSV書き出し・読み込み（読み込むと丸ごと置き換え） |
...
**長押し＝琥珀色／連続押し＝青／それ以外＝赤**の3色が見分けられること、
**画面に出す文字（l1/l2）が1つも無い操作は完了してもLCDが変わらず、`結果コメント`だけが案内エリアに出ること**、
待機中の画面（`simIdleL1`/`simIdleL2`、`{time}`トークン）を設定タブで書き換えられること、
設定タブのコマンド編集が折りたたみ式で開閉できること、CSVの書き出し・読み込みが往復できること、
