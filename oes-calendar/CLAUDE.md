...
- 日本語UI。**配色は「濃紺（メイン `#003DA5`）× オレンジ（アクセント `#F2600D`）」の2色（バイカラー）を維持する。**
  （利用者の指定により、以前の「青×空色」から変更済み。以前の色に戻さないこと。）
  ヘッダー・選択中タブ・カード見出しの番号丸は濃紺×オレンジの2色、カードの見出し帯・
  ボタンの薄色部分などの淡色背景はオレンジ寄りの暖色の薄いトーン（`--blue-l` `--blue-l2` `--line` `--line-w`）
  にする。危険操作（削除）の赤・自動生成タグの緑・手動編集タグの琥珀色などの意味付き色はこの2色ルールの
  対象外（そのまま維持してよい）。ロゴ由来のウェルシス株式会社バッジ（`.welsys-badge`）も対象外
  （実在の会社ロゴの色をそのまま使う）。
...
- **カレンダー画面は「次に何をすればよいか」を常に示す。** 各カード（`.step-card` / `#step1`〜`#step4`）の見出しに
  進捗バッジ（`#badge1`〜`#badge4`。`0 / 3件` のような達成数）を出し、いま取り組むカードに `now` クラスで枠を付ける。
  その項目が終わったら次のカードへ進む案内（`.step-next` / `#next1`〜`#next3`）を出し、
  「次へ」ボタンで次のカードへスクロールして最初の入力欄にフォーカスする（`updateWizardNav()` / `goStep()`）。
  入力のたびに `updateWizardNav()` を呼ぶこと。
- **「カレンダー」画面は質問形式（ウィザード）で、①業態と稼働件数 → ②日付と時刻 → ③店舗名と住所 →
  ...
  - ②「日付と時刻を選ぶ」: `#date-rows`に稼働件数ぶんの行（`.dt-row`）が並び、各行が**日付（`<input type=date>`）と
    開始・終了の時刻プルダウン（`.dr-start` / `.dr-end`）**を独立して持つ。同じ日を複数件でも、別々の日でもよい。
    **午前/午後のトグルは廃止した。時刻を直接指定する方式に戻さないこと。**
...
- **カレンダー画面①のカードにも「クリア（最初からやり直す）」を置く**（`clearWizardAll()`、`#btn-cal-clear`）。
  ...
  **④の登録リスト（`entries`）はこのボタンの対象外**（そちらは既存の「全て削除」で扱う）。
...
- **作業当日タブの①②③の各「コピー」ボタンの隣に「Chat」ボタンを置く。** 押すと、その作業の**業態＋店舗名**
  ...
  **該当する登録が無い場合は、`prompt()`等で案内せずそのまま一般のGoogle Chat（`https://chat.google.com/`）を開く。**
  自動登録プロンプトは廃止済み。
  **同じ店舗のあいだは `.closed` を見て開き直し可否を判定しないこと。** Google Chatは別オリジンのため、
  クロスオリジンの保護によって `.closed` が実際とは無関係に `true`（や例外）を返すことがある。
  それを信じて「閉じている」と判定し開き直すと、せっかく開いていた部屋のページが一覧に戻ってしまう
  （利用者から実際に報告された不具合）。そのため `openStoreChat()` は、同じ `key`（業態＋店舗）である間は
  `chatWin` の有無だけで判断し、`.closed` の状態に関わらず `focus()` のみを行い、開き直さない。
...
- 登録は設定の「詳細設定」内の一覧（`#store-chat-list`）から行う。
  作業当日タブにあった「Chatを登録」ボタンは利用者の指示で廃止した（復活させないこと）。
- **PWA対応（ホーム画面に追加できる）。** Androidで**Chromeのマークが付かない独立したアプリ（WebAPK）**として
  ...
  **PWAアイコンは `index.html` と同じ階層に置く**（`icon-192.png` `icon-512.png` `icon-512-maskable.png`）。
  ...
  `assets/device-printer.jpg`（プリンター写真そのものは変更しない）に**濃紺のバッジ**＋
  白太字の「OES」を重ねたもの（`icon-*.png` に元データを同梱。バイカラーの節を参照）。
  **バッジの文字はオレンジではなく白にする**（オレンジ文字は濃紺の上で見づらいと利用者から指摘済み。
  白地に濃紺の写真＋濃紺バッジ＋白文字、という構成を維持し、オレンジに戻さないこと）。
...
1. **📅 カレンダー** — 質問形式のウィザードで、日付選択・入力・Googleカレンダーへの登録までをこの1画面で完結する
   ...
   「重要なルール」を参照。
   - ①「業態と稼働件数を選ぶ」カード
   - ②「日付と時刻を選ぶ」カード: 件数ぶんの行に日付・開始時刻・終了時刻を入力
