...
- **「📋 コマンド一覧」タブは、実機（SEIKO SA-2610 マルチステーション）の操作パネルのシミュレーター。**
  ...
  - **画面は1枚のカードにまとめ、「①」「②」のような番号付きの区切りは置かない**（以前は
    ...
    カード内の並び順は、シミュレーター本体 → **すぐ下**に「調べたい操作」のプルダウン（`#cmd-select`。
    利用者の指示「画面シミュレーターのすぐ下に調べたい操作のプルダウンを配置して」） → 役立つ場面・説明 →
    手順チップ・再生ボタン・秒数設定・簡易手順（この4つは「まとめて表示できるようにして構わない」という
    利用者の言葉どおり、厳密な区切りを設けず`.cmd-guide-body`でひと続きに表示する）。
  ...
  - **画面・LED・枠の配色は、利用者が添付した実機写真（黒枠・うす緑がかった液晶・緑のLED表示）に
    ...
    - `.sim-lcd-box`（実際の表示部）だけを**うす緑がかった地色**（`#C7D5A0`）にし、文字は
      黒に近い濃緑（`#16240C`）のまま。**表示部の固定サイズ（幅224px・高さ64px。後述の
      サイズ調整で270px・54pxから変更）はこの配色変更自体では変えていない**
      （「サイズが変わるのは嫌」という以前からの指定を維持）。
    - **枠の余白（`.sim-top`のpadding/border）は最小限にしてある。** 電源スイッチ（後述）と並べても
      固定幅224pxの表示部と衝突して横スクロールが出ないよう、意図的に薄い枠にしている
      （厚くすると`390px`幅のスマホで横スクロールが出る不具合が実際に発生したため）。
  - **操作の入り口は2つあり、どちらも同じ手順データを使う。**
    ...
    **シミュレーターの動き（実質的な「動画」）だけでなく、手順を上から順番に読める文章のリストも
    右側に出す**（`#cmd-textlist` / `renderCmdTextList()`。手順チップ・再生ボタン・秒数設定のブロックを
    `.cmd-guide-body`で左右2列にし、左＝実機シミュレーター用の操作、右＝簡易手順のテキスト。
    画面が狭いときは縦に並ぶ）。
  ...
  - **表示部（`.sim-lcd-box`）の大きさは内容に関係なく常に固定**（`width:224px`を`.sim-screen`に、
  ...
  - **画面の枠外に「電源スイッチ」のイメージを置いた**（`.sim-pwrsw` / `#sw-rocker`）。
    **利用者の依頼**：「画面の枠外で構わないから、電源のオンオフスイッチみたいな画面のイメージを
    ...
    ③に対応するため、表示部を270px→224pxに縮め（後述のLCDサイズ調整と共通）、`.sim-top`
...
まとまっていること**、**「調べたい操作」のプルダウンがシミュレーターのすぐ下にあること**、
**画面の枠（`.sim-top`）が黒く、LEDが緑色、LCD（`.sim-lcd-box`）がうす緑がかった地色に黒文字で
表示されること**、**電源スイッチが、画面の幅にかかわらず常にシミュレーターの左に横並びになり、
アプリの標準テスト幅（390px）で横スクロールが出ないこと**（`.sim-top`/`.sim-panel`/
`.sim-with-switch`に余計な余白を戻すと固定幅224pxの表示部と衝突して横スクロールが出るので注意。
375px以下のごく狭い画面は対象外）、**`document.body.scrollWidth`だけでなく、`.sim-panel`が
