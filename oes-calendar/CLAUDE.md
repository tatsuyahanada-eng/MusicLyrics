...
- **「📋 コマンド一覧」タブは、実機（SEIKO SA-2610 マルチステーション）の操作パネルのシミュレーター。**
  ...
  - **設定タブの編集UIは折りたたみ式**（`.gy-item`/`.gy-head`/`.gy-body`を再利用、開閉状態は`cmdOpenSet`
    ...
    見出しの残り（キャレット・「コマンドN」ラベル）をクリックすると詳細（説明・手順・CSVボタン）が開く。
  ...
  - **CSVで一括登録・書き出しができる**（`exportCommandsCsv()` / `importCommandsCsv()` / 手書きの`parseCsv()`）。
    列は「コマンド名,説明,ボタン1,ボタン2（同時押し）,長押し,連続回数,画面1行目,画面2行目,結果コメント」で、
    同じ「コマンド名」の行が連続していると1つの操作としてまとめる。書き出しはBOM付き（Excel文字化け対策）。
    **読み込むと`settings.commands`は丸ごと置き換わる**（`confirm()`で確認してから）。待機画面はCSVの対象外。
  - **`.sim-panel`まわりの見た目は実機の再現なので、アプリの2色ルール（濃い灰色×濃い赤）の対象外**
    （機器写真やウェルシスのロゴと同じ扱い。ゴールドの`メニュー`/`実行`/`シフト`キーなどは実機の色）。
...
| `simSetScreen(l1,l2)` / `simSetMsg(msg)` / `simSetNote(memo)` | シミュレーターのLCD表示（上下行）・案内メッセージ・結果コメント欄 |
| `simDefaultScreen()` / `curIdleScreen()` / `curIdleId` / `applyIdleTokens(s)` | 待機中の画面（複数登録から選ばれている1つを表示。`{time}`置換） |
| `renderIdleSelect()` / `onIdleSelect(v)` | シミュレーター側の待機画面プルダウン（`#sim-idle-view`）の描画と切り替え |
| `renderCmdSelect()` / `onCmdSelect()` / `renderCmdSteps()` | 手順のプルダウンとチップ（9×2 → 8 → 実行 → 完了）の描画 |
...
| `exportCommandsCsv()` / `importCommandsCsv(text)` / `parseCsv(text)` | コマンド一覧のCSV書き出し・読み込み（読み込むと丸ごと置き換え。待機画面は対象外） |
...
**長押し＝琥珀色／連続押し＝蛍光ブルー／それ以外＝赤**の3色が見分けられること、
