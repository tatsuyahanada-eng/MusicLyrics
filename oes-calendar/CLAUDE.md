...
- **「カレンダー」画面は質問形式（ウィザード）で、①業態・稼働件数 → ②件数ぶんの日付・時間帯・店舗名・住所を入力 →
  ③一覧で確認して登録、の3ステップで1画面に完結する。**
  - ①「どこの業態を、何件まわりますか？」: `#w-gyotai`（業態）・`#w-count`（稼働件数、1〜30件から選ぶ）のみ。
    **担当者・AM/PMの共通選択はここには置かない**（担当者は業態の既定担当者を自動で使う。AM/PMは②で1件ごとに選ぶ）。
    選んだ業態の午前・午後それぞれの時刻を`#w-time-hint`に表示するだけの参考情報とする。
  - ②「日付・時間帯・店舗名を入力」: `#w-count`で選んだ件数ぶん、`#store-rows`に行が並ぶ（`renderStoreRows()`が
    件数変更のたびに行を過不足なく増減する）。**各行が独立して日付・午前午後・店舗名・住所を持つ**
    （1件目は10/1の午前、2件目は10/10の午後、のように行ごとに違う日でもよい、という要件のための設計）。
    「このN件を登録」（`submitStoreRows()`）を押すと、**日付が入力されている行だけ**を登録リストに追加する
    （日付が空欄の行は黙ってスキップし、件数はトーストで知らせる）。店舗名・住所は空欄でも登録してよい。
    業態は①で選んだものを全行共通で使う（行ごとに業態を変える機能は無い）。登録後は各行を空にして次の入力に備える
    （`resetStoreRows()`）。
  - ③「一覧で確認して登録」: 登録リストは既定で折りたたみ表示（`.entry`に`open`クラスが無ければ本文非表示）にして、
    件数が多くても一覧が長くなりすぎないようにする（`toggleEntry(id)`で開閉、見出し行に要約と「登録」ボタンを常時表示）。
...
1. **📅 カレンダー** — 質問形式のウィザードで、日付選択・入力・Googleカレンダーへの登録までをこの1画面で完結する
   ...
   「重要なルール」を参照。
   - ①「どこの業態を、何件まわりますか？」カード: 業態・稼働件数（1〜30）を選ぶ。
   - ②「日付・時間帯・店舗名を入力」カード: 件数ぶんの行（日付・午前午後・店舗名・住所）に入力し、
     「このN件を登録」でまとめて③へ追加する。
...
| `onWCountChange()` / `renderStoreRows()` | 稼働件数の変更に合わせて②の入力行を過不足なく増減する |
| `wizSlotFor(ap)` | 現在選択中の業態について、'am'/'pm'に対応する時間帯（`slots[0]`/`slots[1]`）を返す |
| `storeRowHtml(i)` / `onRowAmPmChange(el)` | ②の1行分のHTML生成／行ごとのAM-PMトグルの見た目切り替え |
| `submitStoreRows()` / `resetStoreRows()` | ②の全行を読み取り、日付が入っている行だけ登録リストへ追加／入力後に行を空へ戻す |
