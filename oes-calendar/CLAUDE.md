...
- **「📋 コマンド一覧」タブは、実機（SEIKO SA-2610 マルチステーション）の操作パネルのシミュレーター。**
  ...
  - **操作の入り口は2つあり、どちらも同じ手順データを使う。**
    ...
    **「順番に再生」の間隔は通常1150ms・連続押し1回ぶん650ms・押しまま800ms・長押し2200ms**
    （`cmdPlay()`内の`setTimeout`の数値。「動画（自動再生）が少し早い」という声を受けて、
    元の850/480/500/1600msから読む余裕を持たせて伸ばした。速すぎる／遅すぎると感じたらここを調整する）。
