...
- **「📋 コマンド一覧」タブは、実機（SEIKO SA-2610 マルチステーション）の操作パネルのシミュレーター。**
  ...
  - **表示部（`.sim-lcd-box`）の大きさは内容に関係なく常に固定**（`width:250px`を`.sim-screen`に、
    ...
    **固定サイズの具体的な数値（幅・高さ・`LCD_FONT_MAX`）は調整してよい**（サイズ自体を動的に
    変えないというルールと、決め打ちの数値をずっと変えないというルールは別）。以前は
    幅270px・高さ54px・15pxだったが、利用者の指定「右側の余白が少し気になるので、少し縮めて、
    縦にはもう少し広げることができるので調整してほしい。画面はなるべくピッタリにしたい。
    縦を広げたら、文字も少し大きくするとよい」を受けて、幅を少し縮め・高さと文字サイズを
    大きくする方向に調整した（短い文字列のときに右へできる空白を減らしつつ、視認性を上げる）。
  ...
  - **画面の枠外に「電源スイッチ」のイメージを置いた**（`.sim-pwrsw` / `#sw-rocker`）。
    **利用者の依頼**：「画面の枠外で構わないから、電源のオンオフスイッチみたいな画面のイメージを
    ...
    ③に対応するため、表示部を270px→250pxに縮め（後述のLCDサイズ調整と共通）、`.sim-top`
    `.sim-panel``.sim-with-switch``.sw-rocker`の余白・枠線を切り詰めて、アプリの標準テスト幅
    である390pxでも横スクロールが出ないようにした（それより極端に狭い画面は対象外）。そのため：
...
`.sim-with-switch`に余計な余白を戻すと固定幅250pxの表示部と衝突して横スクロールが出るので注意。
375px以下のごく狭い画面は対象外）、
