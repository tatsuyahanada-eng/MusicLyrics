...
- **「📄 レポートの見方」タブは、実機が印字する「MONSTERA インストールレポート」の読み方を
  ...
  - **レポートは章ごとに分けず、実機の印字どおり1本の縦長でそのまま出す**（利用者の指定：
    ...
    並べてから描画する（`renderRepAll()`）。どの固まりを見ているかだけ分かるよう、印字例の中に
    控えめな見出し（`.rg-blk`）を挟むが、**レポートそのものの桁は動かさない**。
  - **見た目は「キッチンプリンターが打ち出したレシート」にする**（利用者の指定：
    「インストール情報はキッチンプリンターから印刷されるレシートみたいな印字なので、画面のデジタル
    チックな画面というよりは、もう少し印刷物のイメージで作成してほしい」「TOPはこのイメージを採用して」
    ＝利用者が添付した実機印字の写真）。**印字例(`#rep-sample`)は白黒のみ**（`.rg-tok`の地色は
    塗らず、押せる値には薄い下線だけを付ける）。周りは薄いグレーの台紙（`.rg-sample-wrap`）にして、
    その上に白い紙（`.rg-sample`。`max-width:560px`で中央寄せ、薄い影）が乗っている構図にする。
    **各固まりの見出しは、写真そのまま「■を敷き詰めた行 → ■で挟んだ見出し文字 → ■の行」を再現する**
    （`repBannerHtml()`。`.rg-banner-row`/`.rg-fill`は`repeating-linear-gradient`で■を敷き詰め、
    幅が変わっても崩れない。固定個数の■を並べる方式にしないこと）。
...
レポートの見方タブは、**7つの固まり（インストール情報／システム情報／ECT／MST／MPR／MD／HT）が
...
**各固まりの見出しが写真どおり「■の行→■で挟んだ見出し→■の行」の3行（`repBannerHtml()`）に
なっていること**、**マウスを乗せた/タップした値・行が黒塗りではなく蛍光ペンで
マークしたような見た目（黄色の地色＋下線）になっていること**、
**広い画面でスクロールしても、`.rg-detail`（説明）だけが追従し、右側の項目一覧
（`.rg-fields-wrap`）は普通に流れて`.rg-side`全体が画面からずれて浮いたりしないこと**
