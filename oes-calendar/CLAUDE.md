...
2. **🛠️ 作業当日** — **Googleカレンダーの説明文を貼り付けるのが主動線**。画面の先頭に3ステップの案内
   （`.paste-lead`）を置き、貼り付け欄と大きな実行ボタンを最初に見せる。押すと入店/中間報告/退店に
   自動で振り分けられ、各欄のコピー → 隣の「Chat」ボタンでその業態・店舗のGoogle Chatを開いて貼り付ける。
   「このアプリの登録リストから探す」は補助的な導線として控えめに置く。
...
### 作業当日タブの方針
- **貼り付け方式を主役にする。** 各自が自分のGoogleカレンダーの説明文をコピーして貼り付ければ使えること。
  この動線が目立たなくなるような画面変更をしないこと。
