...
- **「カレンダー」画面は質問形式（ウィザード）で、①業態・稼働件数・時間帯 → ②カレンダーで日付をタップ → ③一覧で確認して登録、
  の3ステップで1画面に完結する。** 旧版にあった「複数日を選択してから『まとめて適用』ボタンを押す」方式や、
  日付の複数選択（ハイライト保持）・Shift範囲選択には**戻さない**。日付は毎回タップした瞬間に確認ポップアップが開く
  一発アクション方式にする。
  - ①: `#w-gyotai`（業態）・`#w-count`（稼働件数、1〜30件から選ぶ）・`#w-staff`（担当者）・`#w-ampm`（午前/午後の
    トグル、**既定は午前**）。午前は業態の1つめの時間帯（`slots[0]`）、午後は2つめの時間帯（`slots[1]`、無ければ
    1つめにフォールバック）を使う（`wizSlot()`）。
  - ②: カレンダーの日付をタップすると`openStoreModal(ds)`が呼ばれ、①の内容を踏まえて確認ポップアップ（モーダル）を
    開く。ポップアップには稼働件数ぶんの「店舗名・住所」入力欄が並び、「このN件を追加」で登録リストにN件まとめて
    作成する（`confirmStoreModal()`）。店舗名・住所が両方空欄の行は作成しない。カレンダーのマスには選択状態ではなく
    「その日に何件登録済みか」の件数バッジ（`.day.has .cnt`）を表示する。
  - ③: 登録リストは既定で折りたたみ表示（`.entry`に`open`クラスが無ければ本文非表示）にして、件数が多くても一覧が
    長くなりすぎないようにする（`toggleEntry(id)`で開閉、見出し行に要約と「登録」ボタンを常時表示）。
    「Googleカレンダーで順番に登録」（`startQueue()`）と「ICSファイルで一括取込」（`downloadIcs()`）の両方を
    ここに置く。登録リスト自体は今後の要望次第で外してもよい補助的な確認用一覧という位置づけ（③の役目が
    「一覧で確認できること」である限り、実装手段はここに書いた形に縛られない）。
...
- **作業当日タブの①②③の各「コピー」ボタンの隣に「Chat」ボタンを置く。** 押すと、その作業の店舗名
  （`dayCtx.tenpo`）に対応するGoogle ChatのURLを新しいタブで直接開く（`openStoreChat()`）。URLは
  `settings.storeChats[]`（`{id, tenpo, url}`）に店舗名ごとに保存し、**未登録の店舗では初回だけ`prompt()`でURLを
  聞いて自動的に登録**する（２回目以降は聞かずに直接開く）。目的は「Google Chatの一覧から毎回探す手間をなくす」こと。
  設定の「詳細設定」に一覧（`#store-chat-list`）を置き、事前登録・修正・削除もできるようにする。
...
1. **📅 カレンダー** — 質問形式のウィザードで、日付選択・入力・Googleカレンダーへの登録までをこの1画面で完結する
   ...
   「重要なルール」を参照。
   - ①「どこの業態を、何件まわりますか？」カード: 業態・稼働件数（1〜30）・担当者・午前/午後を選ぶ。
   - ②「作業日をタップしてください」カード: カレンダー（PC/スマホとも常に当月1か月のみ表示、
     `#cal-months`はPCで最大380px）。日付をタップすると①の内容に基づく確認ポップアップが開く。
   - ③「一覧で確認して登録」カード: 登録リスト（既定は折りたたみ）。「Googleカレンダーで順番に登録」
...
3. **⚙️ 設定** — 業態・時間帯・定型文、担当者と担当者定型文（＋各自のカレンダー連携URL）、店舗ごとのGoogle Chat URL、
   作業当日の目印、共通のカレンダー連携（フォールバック）、共通設定、書き出し/読み込み
...
| `renderCalendars()` | カレンダー描画（PC・スマホとも常に1ヶ月）。日付マスには選択状態ではなく登録済み件数バッジを出す |
| `renderWizardForm(keepValues)` | ①業態・稼働件数・担当者のプルダウンを描画（`onWizGyotaiChange` / `onWizAmPmChange` / `updateWizardHints` から呼ばれる） |
| `wizAmPm()` / `wizSlot()` | 現在選択中のAM/PMと、それに対応する業態の時間帯（`slots[0]`/`slots[1]`）を返す |
| `onDayClick(ev, ds)` / `openStoreModal(ds)` | ②日付タップで確認ポップアップを開く（業態未選択ならトースト） |
| `confirmStoreModal()` | ポップアップの店舗名・住所N件分を読み取り、登録リストにN件まとめて作成する（空欄行はスキップ） |
...
| `openStoreChat()` / `findStoreChat(tenpo)` / `upsertStoreChat(tenpo,url)` | 店舗ごとのGoogle Chatを直接開く／検索／登録。未登録なら`prompt()`で聞いて自動保存 |
| `renderStoreChatList()` ほか | 設定画面での店舗Chat URLの追加・編集・削除 |
...
確認観点: ①業態・稼働件数・AM/PMの選択、②日付タップでの確認ポップアップとN件一括作成、③一覧の折りたたみ表示・
Google登録・ICS一括取込、定型文の編集反映、localStorage保存と復元、ICSの中身、作業当日の振り分け・コピー・
店舗別Chatボタン、機器台数プルダウン、横スクロールが出ないこと、JSエラーが出ないこと。
