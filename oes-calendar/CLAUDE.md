...
- **作業当日タブの①②③の各テキスト欄の直下に「コピー」「Chat」ボタンを置く**（ラベルの隣ではない。
  利用者の指定：ラベル横に置くと、隣の欄のボタンと近すぎて誤タップしやすいため、各欄の入力内容の
  すぐ下＝その欄の最後の要素として配置する。3つの欄を続けて縦に並べても誤タップしないよう、
  ボタンの後にしっかり余白（`.field.msg-field`の`margin-bottom:20px`）を取ること）。
  押すと、その作業の**業態＋店舗名**（`dayCtx.gyotai` / `dayCtx.tenpo`）に対応するGoogle ChatのURLを
  新しいタブで直接開く（`openStoreChat()`）。URLは`settings.storeChats[]`（`{id, gyotai, tenpo, url}`）に
  業態＋店舗名の組み合わせで保存する（同じ店舗名でも業態が違えば別ルームのことがあるため、店舗名だけで
  なく業態も含めて照合する＝`findStoreChat(gyotai,tenpo)`）。
  **該当する登録が無い場合は、`prompt()`等で案内せずそのまま一般のGoogle Chat（`https://chat.google.com/`）を開く。**
  自動登録プロンプトは廃止済み。
- **Chatは毎回新しいタブを開かず、名前付きウィンドウ（`oes-store-chat`）1枚を使い回す
  （`window.open(url, 'oes-store-chat')`。同じ名前のウィンドウがすでにそのURLを開いていれば
  ブラウザ側で再読み込みは起きない）。この動作を「毎回 `_blank` で開く」に戻さないこと。**
- **`openStoreChat()`の実装を「賢く」しようとしないこと。過去に3方式を試し、いずれも実機で
  問題が確認済み**（詳しい経緯はコード内のコメントに残してある。要約すると: ①保存したJS参照
  `chatWin`に`.focus()`だけ呼ぶ→Google ChatのCOOP（Cross-Origin-Opener-Policy）で参照が
  ブラウジングコンテキストグループごと切り離され、無反応になる。②`window.open('', name)`で
  空URL取得→一部のモバイル環境のポップアップブロッカーに無反応にされる。③`document.hasFocus()`で
  focus成否を自動判定→実際に正常動作した場合でもtrueのままの環境があり誤判定する）。
  **ポップアップ側の状態はCOOPの仕様上こちらのJSから正しく検知できない**ため、
  自動判定を諦め、**毎回 `window.open(url, 'oes-store-chat')` に実際のURLを渡して呼ぶ**、
  最も単純で確実に反応する方式で確定している。
  **既知の制約（直せない）**: 利用者がポップアップ内で別のスレッドへ手動移動したあとにもう一度
  ボタンを押すと、登録したURLへ戻ってしまう（ブラウザの仕様上、こちらから「開き直さず前面に
  出すだけ」を確実に行う方法が無いため）。緩和策として、`settings.storeChats[].url`には
  ルーム/一覧のURLではなく**スレッド個別のリンク**を登録するよう利用者に案内している
  （マニュアル・設定画面のヒント参照）。
...
| `openStoreChat()` / `findStoreChat(gyotai,tenpo)` | 業態＋店舗のGoogle Chatを、名前つきウィンドウで毎回`window.open(url,name)`して開く／検索。該当なしなら案内せず一般のChatを開く |
