...
- **「📋 コマンド一覧」タブは、実機（SEIKO SA-2610 マルチステーション）の操作パネルのシミュレーター。**
  ...
  - **操作の入り口は2つあり、どちらも同じ手順データを使う。**
    ...
    **「順番に再生」の間隔は既定で通常1200ms・連続押し1回ぶん650ms・押しまま800ms・長押し2200ms**
    （`cmdPlayIntervals()`。「動画（自動再生）が少し早い」という声を受けて、元の850/480/500/1600msから
    読む余裕を持たせて伸ばした上で、**`#cmd-play-speed`（②カードの「次に進むまでの秒数」、0.8〜3秒）で
    見る人がその場で変更できるようにした**。`setCmdPlaySpeed(v)`が`cmdPlayStepSec`（通常間隔の秒数、
    共有設定には入れない）を書き換え、他の3つの間隔は既定の1.2秒との比率で一緒に伸び縮みする。
  ...
  - **「押しまま」（`stayDown`）は、長押し・連続と排他な3つ目の手順タイプ**：離すのを待たずに、
    ...
    案内できるようにするための機能。押した瞬間に`cmdTryAdvance()`を呼んで進めるが、**そのキーの
    `simUpKey()`（実際に離したとき）では二重に進まないよう`simStayMatched`フラグで抑制する**
    （このフラグを忘れると、押しままで進めたステップを離した瞬間にもう一度「押した」と誤検出する）。
