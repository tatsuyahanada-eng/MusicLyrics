...
- **作業当日タブに「クリア（最初からやり直す）」を置く**（`clearDayAll()`）。店舗名・連絡文・機器台数・
  入店連絡の選択・貼り付け欄・下書きをすべて消して初期状態に戻す。
- **①入店連絡に足す文は選択式にする。** `settings.entryNotes`（`[{id, text}]`）の文を入店連絡欄の上に
  チェックボックス（`#entry-note-row`、複数選択可）で並べ、**チェックした文だけ**を入店連絡の1行目（見出し）の
  直後に設定の並び順で差し込む（`rebuildEntryNoteBlock()`）。機器台数と同じ「選んだものだけ入れる」方式。
  初期値は POSレジ清算済み／POSレジ清算 今から開始をお願いいたしました／高所の為、電源ケーブル流用させて頂きます／
