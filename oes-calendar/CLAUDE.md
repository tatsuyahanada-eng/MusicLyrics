...
- **「📋 コマンド一覧」タブは、実機（SEIKO SA-2610 マルチステーション）の操作パネルのシミュレーター。**
  ...
  - **手順は共有設定に入れる**（`settings.commands` = `[{id, name, note, steps:[{id, keys, hold, display}]}]`）。
    管理者が設定タブで追加すれば全員に反映される。`keys`は`SIM_KEYS`のidの配列で、**2つ入っていれば同時押し**、
    `hold:true`なら**長押し**、`display`はそのボタンを押したときに画面（LCD）へ出る文字。
  ...
  - **操作の入り口は2つあり、どちらも同じ手順データを使う。**
    ①プルダウンで操作を選ぶ → チップで「9 → 9 → 8 → 実行 → 完了」と手順が出て、「順番に再生」で
    自動再生、「1つ進む」で1ステップずつ、実機のキーを自分で押しても進む（`cmdGo()` / `cmdPlay()`）。
    ②プルダウンを使わず自由にキーを押す → 押した履歴（`simInput`）と登録済みコマンドを照合し
    （`simMatchProgress()`）、**途中まで一致していれば途中の画面、最後まで一致すればその操作の画面**を出す。
...
| `simSetScreen(l1,l2)` / `simDefaultScreen()` / `simSetMsg(msg)` | シミュレーターのLCD表示と、その下の案内メッセージ |
| `renderCmdSelect()` / `onCmdSelect()` / `renderCmdSteps()` | 手順のプルダウンとチップ（9 → 9 → 8 → 実行 → 完了）の描画 |
| `cmdGo(i)` / `cmdNext()` / `cmdPlay()` / `cmdStop()` / `cmdReset()` | 手順ガイドの進行（1つ進む・自動再生・最初から） |
| `normalizeCommands(raw)` / `defaultCommands()` | コマンド一覧の正規化と初期サンプル（保守機能モード／インストール情報／電源を切る） |
