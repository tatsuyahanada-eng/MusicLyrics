...
- **「📋 コマンド一覧」タブは、実機（SEIKO SA-2610 マルチステーション）の操作パネルのシミュレーター。**
  ...
  - **画面・LED・枠の配色は、利用者が添付した実機写真（黒枠・うす緑がかった液晶・緑のLED表示）に
    ...
    - `.sim-lcd-box`（実際の表示部）だけを**うす緑がかった地色**（`#C7D5A0`）にし、文字は
      黒に近い濃緑（`#16240C`）のまま。**表示部の固定サイズ（幅270px・高さ54px）はこの配色変更でも
      変えていない**（「サイズが変わるのは嫌」という以前からの指定を維持）。
    - **枠の余白（`.sim-top`のpadding/border）は最小限にしてある。** 電源スイッチ（後述）と並べても
      固定幅270pxの表示部と衝突して横スクロールが出ないよう、意図的に薄い枠にしている
  ...
  - **表示部（`.sim-lcd-box`）の大きさは内容に関係なく常に固定**（`width:270px`を`.sim-screen`に、
    `.sim-lcd-box`は`height:54px`固定。以前は`flex:0 1 220px`で縮む余地があったのをやめた）。
    折り返しはせず（`white-space:nowrap`）、長い文字は`fitLcdLine()`が`scrollWidth`を見ながら
    文字サイズを`LCD_FONT_MAX`(15px)から`LCD_FONT_MIN`(8px)まで0.5px刻みで縮めて枠内に収める
  ...
  - **画面の枠外に「電源スイッチ」のイメージを置いた**（`.sim-pwrsw` / `#sw-rocker`）。
    **利用者の依頼**：「画面の枠外で構わないから、電源のオンオフスイッチみたいな画面のイメージを
    ...
    `checkStayDownMatch()`）に乗せている。**画面が広い（760px以上）ときはシミュレーター画面の左に
    横並び、狭いときは画面の上に縦積みにする**（`.sim-with-switch`。狭い画面のまま横に並べると、
    固定幅270pxの表示部と衝突して横スクロールが出てしまう不具合が実際に発生したため、`.sim-panel`
    本体が横並びになる760pxの基準に揃えて縦横を切り替えるようにした）。そのため：
...
表示されること**、**電源スイッチが、画面が広いとき（760px以上）はシミュレーターの左に横並び、
狭いとき（390px幅など）は上に縦積みになり、どちらの幅でも横スクロールが出ないこと**（`.sim-top`
に余計な余白を戻すと固定幅270pxの表示部と衝突して横スクロールが出るので注意）、
