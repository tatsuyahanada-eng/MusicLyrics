...
CREATE TABLE IF NOT EXISTS lp_updates (
  ...
  bump_type       VARCHAR(10)  NOT NULL DEFAULT 'minor' COMMENT 'minor=通常の更新（1.1→1.2） / revision=微修正（1.1→1.11） / major=大幅な変更（1.4→2.00）',
  version_override VARCHAR(20)     NULL COMMENT '版数を自動採番せず直接指定する場合の値（空なら自動採番）',
