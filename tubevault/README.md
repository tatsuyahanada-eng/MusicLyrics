...
1. **設定 →「yt-dlp を更新」** — APK に同梱されたバイナリは古く、YouTube の変更に追随できていません。アプリは起動時に週1回自動更新しますが、手動でも実行できます。
2. **設定 →「プレイヤークライアント」** — 既定の「トークン不要を優先」は、yt-dlp のソース上 PO Token を要求しないと宣言されているクライアント（`web_embedded` / `tv_simply` / `mweb`）を順に試します。駄目なら個別のクライアントを一つずつ試してください。
