...
(() => {
  ...
  function renderNav() {
    ...
    if (navReorder && kids.length > 0) {
      ...
      choiceDock.innerHTML = `<div class="tm-choice-label">${depthLabel(navPath.length)}を選択</div>` +
        tripEntry + kids.map((c) => choiceRow(c)).join('');
    } else {
      choiceDock.className = 'tm-choicedock';
      choiceDock.innerHTML = tripEntry;
    }
  ...
  function showCenterToast(text, sub) {
    ...
    centerToastEl.innerHTML = `
      <div class="tm-centertoast-frame"></div>
      <div class="tm-centertoast-corner tl"></div>
      <div class="tm-centertoast-corner tr"></div>
      <div class="tm-centertoast-corner bl"></div>
      <div class="tm-centertoast-corner br"></div>
      <div class="tm-centertoast-tag">COMPLETE</div>
      <div class="tm-centertoast-body">
  ...
  function tripAppendDest(value, boxArg) {
    ...
    row.innerHTML = '<span class="tm-trip-destno"></span>'
      + '<input class="tm-input tm-trip-dest-in" type="text" maxlength="255">'
  ...
  function tripOpenMap() {
    ...
    if (!start) start = travelOrigin;
    const dests = tripGetDests();
    if (!dests.length) { tripMsg('目的地を1つ以上入力してください', true); return; }
    const enc = encodeURIComponent;
    const destination = dests[dests.length - 1];
    const waypoints = dests.slice(0, -1);
    let url = 'https://www.google.com/maps/dir/?api=1&travelmode=driving&origin=' + enc(start) + '&destination=' + enc(destination);
    if (waypoints.length) url += '&waypoints=' + waypoints.map(enc).join('%7C');
    window.open(url, '_blank', 'noopener');
    tripMsg('Googleマップを別タブで開きました。距離を確認して「片道(km)」に入力してください。');
  }
  async function tripSave() {
    ...
    try {
      ...
      showCenterToast(wasEdit ? '更新完了' : '登録完了', 'TRAVEL EXPENSE');
  ...
  // どちらの伝票が適切かは、回答のたびにAIが相談内容・参照した項目から判断する（サーバー側 report_form）。
  // ヘッダーの常設ボタンは、直近にAIが判断した伝票を覚えておいて開く（会話が進むほど的確になる）。
  let aiChatLastReportForm = 'b';
  const SLIP_LABEL = { b: '作業報告書・B', haku: '白伝票' };
  function slipSampleBtnHtml(which) {
  ...
  async function aiChatSend(text) {
    ...
    try {
      ...
      if (d.empty) {
        ...
        if (thinking) { thinking.textContent = note; thinking.innerHTML += slipSampleBtnHtml(aiChatLastReportForm); }
  ...
  function openAiChat(seed) {
    ...
    aiChatMessages = [];
    aiChatSources = [];
    aiChatLastReportForm = 'b'; // 新しい相談では、直前の案件の判断を持ち越さない
    aiChatRenderEmpty();
  ...
  // ヘッダーの常設ボタンは、この会話でAIが直近に判断した伝票を開く（未判断ならb＝作業報告書・B）
  { const b = $('#aiChatSlipBtn'); if (b) b.addEventListener('click', () => openSlipSample(aiChatLastReportForm)); }
  { const b = $('#aiChatReset'); if (b) b.addEventListener('click', () => {
    aiChatMessages = []; aiChatSources = [];
    aiChatLastReportForm = 'b';
    aiChatRenderEmpty();
