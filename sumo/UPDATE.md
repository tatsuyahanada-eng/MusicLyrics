...
引退した力士のデータに `retired: true` を追加するだけで、番付・力士一覧から自動的に除外されます。

```js
{
  id: 'myogiryu',
  name: '妙義龍',
  retired: true,   // ← これを追加するだけ
  rank: '前頭9',
  …
}
```
...
ニックネームは `data.js` 末尾の `NICKNAMES` に追加します。
未登録の力士は、サイト上「なし」と表示されます。
...
複数の愛称があるときは「／」で区切るのが分かりやすいです（例：`'青い目のウルフ／ウクライナの星'`）。
