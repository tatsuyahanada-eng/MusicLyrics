...
## ⭐ 一番ラクな更新方法（推奨）：画面上の「番付編集」ページを使う

新しい番付が発表されたら：

1. サイトの右上ナビから **「番付編集」** をクリック
2. 各力士の「階級」「東/西」をドロップダウンで選択（引退した力士は「引退」にチェック）
3. 上部の「データ最終更新日」「収録時点の説明」も書き換える（例：`令和8年（2026年）七月場所（名古屋場所）後`）
4. **「data.js をダウンロード」ボタン**を押す → 更新済みの `data.js` がダウンロードされる
5. その `data.js` を、サーバの `sumo/data.js` に **FTP で上書きアップロード** するだけ
6. ブラウザを Ctrl+F5（Mac: Cmd+Shift+R）でスーパーリロード

→ data.js を直接編集する必要なし。構文ミスの心配なし。

> 💡 番付編集ページで変更しても、ダウンロードしてサーバに上げるまではサイトには反映されません（あくまでローカルでの編集です）。

---

## data.js を直接編集する方法（こだわりたい場合）
...
トップ画面に「次回 番付発表」のお知らせが表示されます。発表が終わったら、その場所の `banzukeAnnounced` を `true` にしてください（次の場所の予定が表示されるようになります）。
...
const BANZUKE_SCHEDULE = [
  ...
  // 発表済みになったら ↑ を banzukeAnnounced: true に
  …
];
```
