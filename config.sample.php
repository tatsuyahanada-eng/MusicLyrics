...
/* ---------- 自動バックアップ ----------
   ...
   ■ 「アプリを開いたとき」と「サーバーのcron」のどちらで作るか
     この切り替えは、画面の「修正」→「自動バックアップの方法」から
     いつでも変更できます（管理者のみ）。ここの AUTO_BACKUP_ON_ACCESS は、
     まだ一度もその設定を選んでいない（DBに未保存の）ときだけ使われる
     初期値です。通常はこの定数を書き換える必要はありません。

   ■ 時刻を決めて実行したい場合（さくら等の cron）
     cron で次のファイルを実行してください：  backup-cron.php
       例）cd /home/アカウント名/www/casebycase && php backup-cron.php
     cron が動くようになったら、画面の「修正」→「自動バックアップの方法」で
     「サーバーのcron（時間指定）だけに任せる」を選ぶと、
     「誰かが画面を開いたとき」のバックアップは行われなくなります。
     ※ 両方が有効でも二重にはなりません（1日1ファイルのため）。

   ■ cron を使わない場合
     これまでどおり「アプリを開いたときに作成」のままにしておけば、
     正午を過ぎた最初のアクセスで自動作成されます。
*/
define('AUTO_BACKUP_ON_ACCESS', true);  // 画面でまだ選ばれていないときの初期値（通常は変更不要）
