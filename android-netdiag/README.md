...
- **カメラOCR**（Google Lens風）：メモタブの「カメラで文字を読み取る」から起動。Wi-Fiルーターの
  シールなどにかざすと文字を認識し、**SSID / 暗号化キーを自動抽出してコピー**、または全文をメモに追加。
  ※オンデバイス（ML Kit）で処理。SSID/キーは通常ASCIIのため英数字認識で取得できます。
