...
2つの機能を持つ Android アプリです（下部タブで切替え）。

1. **音声入力**：端末のマイクで話した内容をリアルタイムに文字起こし
   （端末標準の `android.speech.SpeechRecognizer` を利用。外部 API キー不要）
2. **定型文**：項目（メイン）ごとに登録した定型文を選び、`日付`（プルダウン）
   `時間1`/`時間2`（プルダウン）`氏名`（テキスト）を差し込んでコピー。
   右上の設定（⚙）から項目・定型文を追加/編集できる。
